...
const { name: moduleName } = JSON.parse(
  await fsReadFile("package.json", "utf-8")
);
const quietRepo =
  "https://raw.githubusercontent.com/claysauruswrecks/quiet-js/master/";
...
async function downloadRequirements() {
  ...
  return Promise.all(
    Object.keys(requirements).map(async (key) => {
...
async function bundle() {
  ...
  log(
    `${profileNames.length} profiles names found in 'quiet-profiles.json', creating 'index.d.ts'`
  );
  const indexDTs = await fsReadFile("template.index.d.ts", "utf8");
  await fsWriteFile(
    "index.d.ts",
    indexDTs.replace(
      /type ProfileName = ['a-zA-Z0-9| ]*;/,
      `type ProfileName = ${profileNames.map((p) => `'${p}'`).join(" | ")};`
    )
  );
